...
def get_transaction(uow: AbstractCategoryUnitOfWork, id: int):
    with uow:
...
def get_list_transactions_by_category_id(
    uow: AbstractCategoryUnitOfWork, id: int
) -> List[Transaction]:
    with uow:
        return uow.transactions.list_by_category_id(id)


def delete_transaction(uow: AbstractCategoryUnitOfWork, id: int):
    with uow:
